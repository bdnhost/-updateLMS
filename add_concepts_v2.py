...
print("=" * 100)
print("🎓 הוספת מושגים מפתח בשלבים")
print("=" * 100)

# Concepts by assignment - in batches per field
assignments_data = {
...
print(f"\n📝 הוספת {sum(len(v.get('key_concepts', [])) for v in assignments_data.values())} מושגים לכל המטלות...")
print("-" * 100)

updated = 0
failed = 0

for assignment_id, data in assignments_data.items():
    try:
...
print(f"   📋 סה״כ: {len(assignments_data)}")
print(f"\n🎯 כל מטלה כעת עם 4-7 מושגים מפתח (Key Concepts)!")
