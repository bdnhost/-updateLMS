"""
🔗 שיוך חומרים לסשנים - עדכון מקיף לכל המערכת
משייך חומרים רלוונטיים לכל הסשנים ללא חומרים (40 סשנים)
"""
...
def calculate_relevance_score(material, session_keywords, course_id):
    """חשב ציון רלוונטיות בין חומר לסשן"""
    score = 0
...
def find_materials_for_session(session, all_materials):
    ...
    if not session_keywords:
        # אם לא נמצאו מילות מפתח, השתמש בחומרים מאותו קורס
    ...
    # מיין לפי ציון והחזר 2-4 הטובים ביותר
    scored_materials.sort(key=lambda x: x[1], reverse=True)

    # קח 2-4 חומרים עם ציון טוב
    top_materials = []
    for material, score in scored_materials[:6]:  # בדוק את 6 הראשונים
        if score >= 20 and len(top_materials) < 4:  # מינימום ציון 20
            top_materials.append(material)

    # אם לא נמצאו מספיק, קח לפחות 2
...
🎯 אחרי העדכון, כל 77 הסשנים יהיו עם חומרים!
""")
